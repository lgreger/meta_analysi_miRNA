...
We further annotate the samples using the annotation file and check if the data is paired. Only 31 samples AHUS Agilent v3 is paired.  No information on matched samples from UCAM.  Since we are interested in DCIS comparisons ( where there is no paired data available) we will not use paired data design.
```{r , eval = FALSE}
...
paired_normals <- grep("N", ahus_v3_targets$FileName, perl = TRUE, value = TRUE)
paired_normals <- gsub("N", "", paired_normals)
paired_tumors <- grep("T", ahus_v3_targets$FileName, perl = TRUE, value = TRUE)
paired_tumors <- gsub("T", "", paired_tumors)
all_s<- c(paired_normals , paired_tumors)
paired_samples <- all_samples[which(duplicated(c(paired_tumors, paired_normals)))]

Subject <- c()
for ( i in 1: length(ahus_v3_targets$FileName)) {
if (ahus_3[i] %in% paste(paired_samples, "T", sep = "")  ||  ahus_3[i] %in% paste(paired_samples, "B", sep = "")) {
Subject <- c(Subject, "1")
} else if  (ahus_3[i] %in% paste(paired_samples, "N", sep = "")) {
Subject <- c(Subject, "2")
} else {
Subject <- c(Subject, "1")
}
}
targets_ahus_v3 <- cbind(ahus_v3_targets ,status,  var2,ihc,  GErep, Subject) 
...
```
## Find common features both datasets
```{r, eval = FALSE}
...
write.table(test, "dcis_to_ all_dcis_HER2neg_ERneg_PGRneg_limma.txt", sep = "\t", quote = FALSE)
...
write.table(test, "dcis_to_ all_dcis_HER2neg_ERpos_limma.txt", sep = "\t", quote = FALSE)
...
write.table(test, "dcis_to_ all_dcis_HER2pos_ERneg_limma.txt", sep = "\t", quote = FALSE)
...
write.table(test, "dcis_to_ all_dcis_HER2pos_ERpos_limma.txt", sep = "\t", quote = FALSE)
```
